...
public class quick
{
    public static void main(String [] args)
    {
        ...
        for(int i=4;i<=67108864;i*=4) {
            ...
            int [] array_q = new int[i];
            int [] array_h = new int[i];
            int [] array_m = new int[i];

            for (int j = 0; j < i; j++) {
                array_q[j] = rand.nextInt(100);
                array_h[j] = array_q[j];
                array_m[j] = array_q[j];
            }

            start_time = System.nanoTime();
            randomized_quicksort(array_q,0,i-1);
        ...
        //part d
        int [] array_ram = new int[67108864];

        for (int j = 0; j < 67108864; j++) {
            array_ram[j] = rand.nextInt(100);

        }
        //run by one by
        randomized_quicksort(array_ram,0,67108864-1);
        //heap_sort(array_ram);
        //merge_sort(array_ram, 0, 67108864-1);
